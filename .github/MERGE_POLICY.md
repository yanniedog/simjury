...
Manual after gates green: `npm run pr:merge -- --pr <n>`.

Do not merge on CI green alone — complete thread resolution per `WORKFLOW.md`.
...
Qwen and reviewer presence are advisory and disabled as automatic gates.
Premature **close** (unmerged) with unresolved review feedback is reversed by
