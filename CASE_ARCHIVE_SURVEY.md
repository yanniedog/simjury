...
```mermaid
flowchart LR
  A[Discover case] --> B{NBT or State Trials?}
  B -->|Yes| C[Archive.org / library copy]
  B -->|No| D[OBO search date only]
  D --> E[Sessions Papers scan dhi.ac.uk/san]
  C --> F[TABULATION.md locators]
  E --> F
  F --> G{Gaps?}
  G -->|Yes| H[BLOCKED_REPORT]
  G -->|No| I[sources.json + JSON authoring]
```
