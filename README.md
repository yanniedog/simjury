...
Narration uses Apache-2.0 Kokoro-82M clips generated by GitHub Actions on free CPU
runners and served from 32 GitHub Releases (with ample room below GitHub's
1,000-assets-per-release limit). Graded built-in American and British Kokoro speakers
supply gendered male/female timbres so each courtroom role stays consistent across a
case. Natural narration requests a deterministic opaque clip id from GitHub. The URL
contains no case text, verdict or private player state, but GitHub can observe the timing
and order of playback requests and map each id to a line in this public corpus. Missing
clips fall back to an English voice advertised as local by the browser, keeping adjacent
speakers distinct when at least two voices exist. Cloudflare never synthesizes or stores
narration.
