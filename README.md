...
no case text, verdict or private player state, but GitHub can observe the request and map
it to a line in this public corpus. Missing clips fall back to an English voice advertised
as local by the browser, keeping adjacent speakers distinct when at least two voices
exist. Cloudflare never synthesizes or stores narration.
