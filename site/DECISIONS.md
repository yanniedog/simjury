...
- **Web decision (owner-directed, updated 2026-07-28):** the web player remains
  **listenable, like a podcast**. Opt-in narration first plays Apache-2.0 Qwen3-TTS
  clips pre-generated by GitHub Actions (VoiceDesign gendered reference bank + Base
  clone) and published in sharded GitHub Releases. Male and female speakers are assigned
  from separate designed voice pools so courtroom roles stay distinctly gendered. If a
  clip cannot play, it falls back to an English voice advertised as local by the browser
  and keeps adjacent speakers distinct when at least two device voices exist.
