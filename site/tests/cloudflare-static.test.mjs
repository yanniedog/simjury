...
test('production deploy requires the exact pinned immutable release', () => {
  ...
  const sourceReview = deploy.indexOf('Recompute and require exact pinned-media source')
  ...
  assert.match(deploy, /--require-pinned-media/)
  assert.match(deploy, /expected_digest=.*review_content_digest/)
  assert.match(deploy, /--expected-digest "\$expected_digest"/)
  assert.match(deploy, /--expected-release-tag "\$RELEASE_TAG"/)
  assert.doesNotMatch(deploy, /--require-approved/)
})
